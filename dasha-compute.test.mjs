...
const kitReadme = await readFile(new URL('./dasha-compute-open-alpha/README.md', import.meta.url), 'utf8');
assert.doesNotMatch(kitReadme, /Neither mode has billing/, 'kit README no live-has-no-billing claim');
assert.match(kitReadme, /Local mode has no billing/, 'kit README local free');
assert.match(kitReadme, /prepaid credits \(\$0\.05\/job\)/, 'kit README live prepaid');
assert.match(kitReadme, /self-route \(own Mac\) stays free/, 'kit README self-route free');
assert.match(kitReadme, /Hosted Ask is 3 free \/ 10 min/, 'kit README Hosted floor');
const kitThreat = await readFile(new URL('./dasha-compute-open-alpha/THREAT_MODEL.md', import.meta.url), 'utf8');
assert.doesNotMatch(kitThreat, /Future balances and metering records, which do not exist in this release/, 'kit THREAT no missing-balances claim');
assert.match(kitThreat, /The local coordinator has no billing ledger/, 'kit THREAT local no ledger');
assert.match(kitThreat, /Live getdasha\.com uses prepaid credits/, 'kit THREAT live prepaid');
...
assert.match(kitFile('README.md'), /OLLAMA_KEEP_ALIVE=-1/);
assert.match(kitFile('README.md'), /Heartbeat advertising can succeed while a mid-Ask \*\*`URLError`\*\*/);
assert.match(kitFile('provider/agent.py'), /def power_soft_report/);
