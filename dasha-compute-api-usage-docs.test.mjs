#!/usr/bin/env node
/**
 * /compute/api + /compute/api/v1 docs must mention usage-on-stream for v1 and Hosted UI.
 */
...
assert.match(html, /Usage on stream · v1 chat\/completions \+ Hosted Ask stop chunk/);
...
assert.match(String((await apiStatus.json()).usage || ''), /see \/compute\/api\/v1/);
