...

{
  ...
  const claims = await requestSessionClaims({ LOBBY_SESSION_SECRET: SECRET }, req);
  ...
  const legacyClaims = await requestSessionClaims({ LOBBY_SESSION_SECRET: SECRET },
...
// Task 17 (#289): privilege changes (e.g. API key creation) rotate the
// session and revoke the old sid — the client must follow the rotated
// Set-Cookie for subsequent calls.
function followRotated(response, label) {
  const rc = response.headers.get('Set-Cookie');
  assert.ok(rc && rc.includes(COOKIE), `${label} rotates the session cookie`);
  const token = decodeURIComponent(rc.split(`${COOKIE}=`)[1].split(';')[0]);
  assert.ok(token.length > 0, `${label} yields a rotated token`);
  return token;
}
...
// The grant was minted from freshToken's sid (Task 17, #289), and the key
// creation below rotates that sid — so this must run before §11, and §11
// must follow the rotated cookie.
{
  ...
  grantTokenValue = followRotated(r, 'grant key creation');
}

// 11. fresh login (<10m auth_time) satisfies the gate — no extra prompt
{
  // §14 rotated the session above; rotation preserves auth_time, so the
  // rotated token is still a fresh wallet login and passes without step-up.
  const keys = await network.fetch(netReq('/compute/api/keys', 'POST', grantTokenValue, { name: 'FreshKey' }), ORIGIN);
  assert.equal(keys.status, 201, 'fresh wallet login creates a key without step-up');
  const rotatedToken = followRotated(keys, 'key creation');
  const pref = await network.fetch(netReq('/compute/api/provider/payout-pref', 'POST', rotatedToken, { method: 'usdc', wallet: WALLET }), ORIGIN);
