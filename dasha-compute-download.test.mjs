...
test('wrong hash, truncated, oversized and unavailable archives fail closed',async()=>{
 for(const reply of [()=>new Response(new Uint8Array(body.length)),()=>new Response(body.slice(1)),()=>new Response(new Uint8Array(body.length+1)),()=>new Response('',{status:404}),()=>{throw Error('offline')}]){
  const r=await computeDownloadResponse(req('/dasha-compute-open-alpha.tar.gz'),reply,config);assert.equal(r.status,503);assert.equal(r.headers.get('Cache-Control'),'no-store');
 }
});
test('metadata matches archive; HEAD has no body; unrelated routes untouched',async()=>{
 ...
 const manifest=await computeDownloadResponse(req('/compute/release.json'),never,config);assert.deepEqual(await manifest.json(),config.manifest);
 ...
 const sidecar=await computeDownloadResponse(req('/dasha-compute-open-alpha.tar.gz.sha256'),never,config);assert.ok((await sidecar.text()).startsWith(config.manifest.sha256));
