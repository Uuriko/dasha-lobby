#!/usr/bin/env node
/**
 * /compute/proof + /compute/proof.json + /compute/proof.md: the six-section
 * live proof page. Pins: routes registered in the worker, every section
 * present, all numbers sourced from live public endpoints or linked external
 * docs, and the honesty invariants (0 renders as 0, dark windows shown, no
 * invented uptime SLA, DeepSeek Sep 14 repricing called out, one-provider
 * wall). proof.md is the Knap markdown twin of proof.json.
...
assert.equal(worker.split('return computeProofPageResponse(request);').length - 1, 2, 'proof route at both dispatch sites');
assert.equal(worker.split('return computeProofJsonResponse(request, env);').length - 1, 2, 'proof.json route at both dispatch sites');
assert.equal(worker.split('return computeProofMdResponse(request, env);').length - 1, 2, 'proof.md route at both dispatch sites');
assert.match(worker, /url\.pathname === '\/compute\/proof\.json'/);
assert.match(worker, /url\.pathname === '\/compute\/proof\.md'/);
assert.match(worker, /computeProofPageResponse/);
assert.match(worker, /computeProofJsonResponse/);
assert.match(worker, /computeProofMdResponse/);
assert.match(worker, /renderComputeProofMarkdown/);
...
assert.match(worker, /Knap template on disk: phase0-publish\/knap/);
assert.match(worker, /text\/markdown; charset=utf-8/);
assert.doesNotMatch(worker, /'\/compute\/doctor\.md'/);
