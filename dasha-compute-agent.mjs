...
name: dasha-compute
description: First call on Dasha Compute. OpenAI-compatible v1 chat/completions on community Macs — fail-loud no_mac_online when none advertise the model. Hosted Ask is POST /compute/api/chat, not a silent v1 swap. Use when probing healthz/network/models or posting chat/completions.
---
...
Receipts: signed, chained, publicly verifiable. Chain: GET https://www.getdasha.com/compute/api/chain · signer key: https://www.getdasha.com/keys.json · machine verdict: https://www.getdasha.com/compute/api/verify · browser: https://www.getdasha.com/verify. GET https://www.getdasha.com/compute/api/receipts is authenticated retrieval for your own key.
...
v1 chat/completions: community Macs. None advertise the model → no_mac_online (poll /compute/api/network · join /compute#provide).
Hosted Ask: browser POST /compute/api/chat (status.live Workers AI). Not a silent v1 swap.
...
${COMPUTE_HOSTED_FLASH_TXT}
${COMPUTE_HOSTED_ASTRA_TXT}
spend caps every key carries its own USD cap - $5/month default, $1-$1,000 or uncapped at creation, monthly or weekly reset; 402 key spend limit reached on exceed - https://www.getdasha.com/caps
