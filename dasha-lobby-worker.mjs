...
export function orderHomeLongPage(html) {
  ...
  for (let n = 0; n < 4; n += 1) {
    ...
    if (!grokDoor.cut) break;
    out = grokDoor.html;
  }
  out = out.replace(/<style\b[^>]*id=["']dasha-siwg["'][^>]*>[\s\S]*?<\/style>/gi, '');
  out = out.replace(/<p class="siwg-credit">[\s\S]*?<\/p>/gi, '');
 ...
 *  Bare leftover /mcp|/compute/mcp (live 308 → /compute) now fold to the catalog.
 *  Must not catch *.json (exact faces stay 200). */
...
const POTTER_COMPUTE_DOCTOR_PROVIDE_308_PATHS = new Set([
  ...
  '/compute/provide/enroll-code', '/compute/provide/enroll-code/',
  '/compute/provide/enroll_code', '/compute/provide/enroll_code/',
  '/compute/provide/enrollcode', '/compute/provide/enrollcode/',
  '/compute/provide/enrol-code', '/compute/provide/enrol-code/',
 ...
 *  Published ASSETS tar (live GET /dasha-compute-open-alpha.tar.gz, repacked 2026-09-22):
 *  sha256 7809ccbf… · 198,182 bytes · 33 files, SOURCE-MANIFEST.sha256 self-verified ·
 *  package.json 0.3.0 · no VERSION file · no openai: backend.
 *  Tip source 0.3.2 (residual_alpha / openai:) stays unpublished until Instinct packs ASSETS. */
...
export default {
  async fetch(request, env) {
    ...
    if ((request.method === 'GET' || request.method === 'HEAD') && (url.pathname === '/lobby' || url.pathname === '/lobby/')) {
      let html = asStandaloneLobbyPage(LOBBY_PAGE_HTML);
      /* Forum first paint parity with the www product edge: the chips + thread list (or one
         thread) are server-painted on the lobby host too, so lobby.getdasha.com/lobby never
         depends on client hydration for the forum. Same fail-open shape: a dead DO still serves
         the page, just unpainted. */
      const threadId = url.searchParams.get('t') || '';
      if (/^[A-Za-z0-9_-]{1,40}$/.test(threadId)) {
        try {
          const data = await publicForumThread(env, threadId);
