...
const POTTER_HOME_308_PATHS = new Set([
  ...
  // Leftover /candles (Worker 66dadebe): live /candles (+slash / Title-case)
  // html-404 → 308 home. Peer of live /chart (mirror PR #63). Do not invent
  // /compute/candles. Never fold /price (200 JSON) or /privacy (200).
  '/candles', '/candles/',
]);
const POTTER_HOWTO_308_PATHS = new Set([
  ...
  '/purchase', '/purchase/',
  // Leftover /sell (Worker 66dadebe): live /sell (+slash / Title-case)
  // html-404 → 308 /how-to-buy. Peer of live /swap /trade (mirror PR #63).
  // Do not invent /compute/sell. Exact /how-to-buy stays 200.
  '/sell', '/sell/',
]);
...
const POTTER_COMPUTE_TAB_308_PATHS = new Set([
  ...
  // Leftover chatgpt/cursor/copilot batch (Worker 66dadebe): live /chatgpt
  // /cursor /copilot (+ /compute/* tabs, slash / Title-case) html-404 → 308
  // /compute. Peers of live /claude /lmstudio /openwebui (mirror PR #69).
  // Fold product doors to plain /compute (no hash). NOT OpenAI/Microsoft product pages.
  // Skip /openai /anthropic /arcade /v1 /x402 /health /status.
  // Never fold /price (200 JSON) or /privacy (200).
  '/chatgpt', '/chatgpt/',
  '/cursor', '/cursor/',
  '/copilot', '/copilot/',
  '/compute/chatgpt', '/compute/chatgpt/',
  '/compute/cursor', '/compute/cursor/',
  '/compute/copilot', '/compute/copilot/',
]);
